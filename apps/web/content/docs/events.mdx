...
| `text.delta`     | Streaming only _(coming next)_          | `delta`                                                    |
...
An async-iterator API for streaming (`for await (const event of agent.stream(…))`),
plus trace exporters for JSON and OpenTelemetry.
