...
the same ones the SDK's own 64-test suite uses, which is why it runs in about
250ms.
...
Yes — but keep it out of the unit suite. Put one or two live tests behind an
environment check so CI stays fast and deterministic:
