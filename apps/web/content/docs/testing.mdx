...
the same ones the SDK's own offline test suite uses, which is why they run in about
...
A [malformed-then-valid](/docs/structured-output) script _is_ the repair path —
no extra mock capability needed:
...
Drop `maxOutputRetries: 0` in to assert the failure instead — it rejects with an
`InvalidOutputError` carrying `issues` and `rawText`.

### Guardrails and approval

The two assertions worth writing: a rejected input costs nothing, and a blocked
tool call leaves the run recoverable.
...
<Callout type="tip">
  ...
  bug the entire offline suite missed: a run-level timeout was being reported as
  a network error, because real `fetch` rejects with the abort **reason** object
  while the mock rejected with a generic `AbortError`.
</Callout>
