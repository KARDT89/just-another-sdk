...
| ----------------- | -------- | --------------------------------------------------------------------------------------- |
| `Agent`           | class    | The agent. `new Agent(config)`, `.run()`, `.clone()`, `.withTools()`, `.toConfig()`     |
...
| `AGENT_DEFAULTS`  | const    | `{ maxTurns: 10, toolTimeoutMs: 30000, modelTimeoutMs: 120000, onToolError: 'return' }` |
...
| `RunStep`    | type     | One turn: text, tool calls, results, usage, timing |
