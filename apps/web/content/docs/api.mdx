...
| ----------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Agent`           | class    | `new Agent(config)`, `.run()`, `.stream()`, `.session()`, `.resumable()`, `.resume()`, `.resumeApproval()`, `.clone()`, `.withTools()`                                           |
...
| `AGENT_DEFAULTS`  | const    | `maxTurns: 10`, `toolTimeoutMs: 30000`, `modelTimeoutMs: 120000`, `onToolError: 'return'`, `maxRetries: 2`, `retryDelayMs: 250`, `maxRetryDelayMs: 10000`, `maxOutputRetries: 1` |
...
| `RunStep`    | type     | One model call: `kind` (`turn`/`repair`/`resume`), text, tool calls, results, usage, timing |
...
| Export                                                                                 | Kind | Description                                          |
| -------------------------------------------------------------------------------------- | ---- | ---------------------------------------------------- |
| `InputGuardrail`, `OutputGuardrail<T>`, `ToolGuardrail<T>`, `AnyToolGuardrail`         | type | The three kinds — see [Guardrails](/docs/guardrails) |
